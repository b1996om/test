...
//자료형을 제한하는 제네릭 클래스
open class Animal(val size: Int) {  //open 키워드로 인해 Animal을 상속받을 수 있게 됨
    fun feed() = println("Feeding...")
}
class Cat(val jump: Int): Animal(50) //Animal 클래스를 상속받은 Cat클래스

class Spider(val poison: Boolean): Animal(1) //Animal 클래스를 상속받은 Spider클래스
